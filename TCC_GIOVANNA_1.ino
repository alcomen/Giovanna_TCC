...
void loop() 
{
  ...
  mqtt.publish("/giovanna/gps/sats", msg);

  sprintf(loc, "%2.6f\,%2.6f", gps.location.lat(), gps.location.lng());
  mqtt.publish("/giovanna/gps/loc", loc);
  
  velocidade = gps.speed.mps();
  sprintf(msg, "%d", velocidade);
  mqtt.publish("/giovanna/gps/vel", msg);
